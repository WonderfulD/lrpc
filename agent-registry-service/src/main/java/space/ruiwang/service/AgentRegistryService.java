...
import space.ruiwang.util.AgentIdentityUtil;
...
public class AgentRegistryService {
    ...
    private SkillDocument buildSkillDocument(AgentCard agent, SkillCard skill) {
        ...
        if (agent.getProvider() != null) {
            document.setAgentOrganization(agent.getProvider().getOrganization());
        }
        document.setAgentVersion(agent.getVersion());
    ...
    private void validateAgentCard(AgentCard agent) {
        ...
        if (endpoint == null || StrUtil.isBlank(endpoint.getUrl())
                || CollUtil.isEmpty(endpoint.getTransport())
                || endpoint.getTransport().stream().allMatch(StrUtil::isBlank)) {
